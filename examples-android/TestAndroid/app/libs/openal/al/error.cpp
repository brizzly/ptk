...
#ifndef __ANDROID__
// Only include the debug header if not building for Android
#include "al/debug.h"

// Debugging-related code here

#else
// For Android, provide empty debugging macros or functions if needed

#include "context.h"
// Use DebugSource here, which is defined consistently across all files
...
AL_API auto AL_APIENTRY alGetError() noexcept -> ALenum
{
    if(auto context = GetContextRef()) LIKELY
        return 0;//alGetErrorDirect(context.get());
