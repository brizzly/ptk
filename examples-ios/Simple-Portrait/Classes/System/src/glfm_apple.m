...
- (void)createDrawable {
    ...
    if (self.multisampling) {
        ...
        if (error != GL_NO_ERROR) {
            NSLog(@"Error after setting renderbuffer storage: %x", error);
        }
...
- (void)prepareRender {
    ...
    if (self.multisampling && _msaaFramebuffer == 0) {
        NSLog(@"Error: Framebuffers or renderbuffers are not initialized properly.");
    }
    
    if(_defaultFramebuffer == 0 || _colorRenderbuffer == 0) {
        NSLog(@"Error: Framebuffers or renderbuffers are not initialized properly.");
    }
    ...
    GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        NSLog(@"Framebuffer is not complete: %x", status);
        return;
    }
